...
def get_avg(lines: [], sibsp: int):
    ...
    for line in lines:
        # Если во время разбора произошла ошибка например
        # конвертация строки в число то пропускаем строку
        try:
            # формируем список из строки с полями,
            # деля строку по разделителю ','
            data = line.split(',')
            # применяем для каждого элемента списка функцию strip()
            data = list(map(str.strip, data))
            # data[6] это поле Age, а data[7] это поле SibSp
            # и SibSp равно фильтру sibsp, то
            if int(data[7]) == sibsp:
                # data[2] это поле Pcalss, суммируем возраст
                # по классу пассажиров
                # суммируем только действительный возраст !=0, >0
                # и класс является только 1, 2 или 3
                if data[6] == "" \
                        or float(data[6]) <= 0 \
                        or data[2] not in ['1', '2', '3']:
                    continue
    ...
    return \
        [
           round(pclass_summ['1']/pclass_count['1']
                 if pclass_count['1'] > 0 else 0.0, 2),
           round(pclass_summ['2']/pclass_count['2']
                 if pclass_count['2'] > 0 else 0.0, 2),
           round(pclass_summ['3']/pclass_count['3']
                 if pclass_count['3'] > 0 else 0.0, 2)
        ]
...
def var7_main():
    ...
    # выводим текст
    # refactoring
    # st.write('Для просмотра данных о среднем возрасте пассажиров по каждому '
    #          'классу обслуживания в соответствии с количеством братьев, '
    #          'сестер, сводных братьев, сводных сестер, супругов на борту,'
    #          'выберите соответствующий пункт из списка')
    # Определяем элемент управления number_input
    ss = st.number_input('Значение поля SibSp:', 0, 8, 0, 1)
    # вычисляем средний возраст пассажтиров по классам
    # в соответствии с фильтром из
    # number_input
    # открываем файл
    with open('data.csv') as file:
        # lines - список строк, читаем в lines файл пропуская заголовок
        lines = file.readlines()[1:]
    # возвращаем среднее значение возраста по классам
    # с учетом заданного фильтра sibsp
